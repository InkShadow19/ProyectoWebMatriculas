...
export class BancoService implements OnDestroy {
    ...
    getList(page: number = 0, size: number = 10, codigo?: string, descripcion?: string, estado?: string): Observable<PageResponse<BancoDto> | undefined> {
        ...
        // Pasamos tanto el código como la descripción al domain service
    ...
    add(body: Partial<BancoDto>): Observable<BancoDto | undefined> {
        this.isLoadingSubject.next(true);
    ...
    update(identifier: string, body: Partial<BancoDto>): Observable<BancoDto | undefined> {
        this.isLoadingSubject.next(true);
